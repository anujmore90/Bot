...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002327019713').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '8047797327').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '8047797327').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002477962995') #Channel / Group Id for force sub ( make sure bot is admin )
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002482323036')) #Log channel id ( make sure bot is admin )
